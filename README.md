...
- **通用格式支持**: 得益于 `image` crate，原生支持 PNG, BMP, TIFF 等多种无损图像格式。
...
```bash
lsb_hide hide --image <原始图像.png> --text <要隐藏的文本.txt> --destination <输出图像.png>
```

**参数说明:**

- `-i, --image`: 原始的无损格式图像文件路径 (例如 .png)。
- `-t, --text`: 要隐藏的文本文件路径。
...
```bash
lsb_hide recover --image <已嵌入信息的图像.png> --text <恢复后的文本文件.txt>
```

**参数说明:**

- `-i, --image`: 包含隐藏信息的图像文件路径。
